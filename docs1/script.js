document.addEventListener("DOMContentLoaded", function () {
    ...
    modeSelection.style.transition = "opacity 0.5s ease";
      ...
      // Remove the existing MQTT client initialization and replace with:
window.isAutomaticMode = false; // Make this global

// Keep existing MQTT message handler but add:
mqttClient.on("message", function(topic, message) {
  const value = message.toString();
  // Update both sections when receiving status updates
  updateDeviceStatus(topic, value);
  updateAutomatedStatus(topic, value);
});

function updateDeviceStatus(topic, value) {
  // Existing status update logic...
}

function updateAutomatedStatus(topic, value) {
  // Similar logic for automated mode section...
}
      // Force reflow
...
navLinks.forEach(link => {
  ...
  // --- MQTT Integration ---
  const mqttBrokerUrl = "ws://139.59.68.181:1883"; // Replace with your IP
  let mqttClient;
