...
const long interval = 50; // Interval between steps (milliseconds)
...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    // Check if this is the fan control topic
    if (strcmp(topic, FAN_CONTROL_TOPIC) == 0)
    {
        // Control fan based on message
...
{
    Serial.begin(115200);
    myStepper.setSpeed(10); // Speed in RPM - increased for better rotation
    setup_wifi();
...
void loop()
{
    ...
    if (fanRunning && currentMillis - previousMillis >= interval)
    {
        previousMillis = currentMillis;
        myStepper.step(1); // Take one step at a time for continuous rotation
    }

    // Only read sensors and send data every 5 seconds
